with RandomFunctions, AsteroidPackage, Ada.Calendar, Ada.Numerics.Float_Random, Ada.Integer_Text_IO, Ada.Text_IO, Ada.Unchecked_Deallocation, GenericProtectedOrderedQueue;
use RandomFunctions, AsteroidPackage, Ada.Calendar,
   Ada.Numerics.Float_Random, Ada.Integer_Text_IO, Ada.Text_IO;
...
procedure Aoption is
   ...
   package AsteroidOrderedQueuePackage is new GenericProtectedOrderedQueue(
      Asteroid_Ptr, Duration, GetAbsoluteImpactTime, AstGTE, 15);
   use AsteroidOrderedQueuePackage;
   ...
   task body TFC is
      ...
      --Data Structures
      AstStack     : ProtectedOrderedQueue;
      Scout        : ScoutProbe;
   ...
   begin
      ...
      while not Recalled loop
         ...
         Ast := new Asteroid;
         Construct(Ast, AstNumber, RandomSize(RandomNum), Duration(
               RandomImpactTime(RandomNum)), Seconds(Clock));
   ...
   begin
      ...
      while not SelfDestruct loop
         ...
         if Available then
            if TimeToDestroy(GetSize(Target), ProbeType) <=
                  RemainingTimeToImpact(Target) then
               --Destroy
