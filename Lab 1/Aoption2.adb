...
procedure Aoption2 is
    ...
    begin
       ...
       case Request is
           when Permission_To_Land =>
               ...
               -- Make decision.
               put(Shuttle); put(" is granted permission to land. (Officer 1)"); new_line;
           when Call_The_Ball =>
               delay Duration(Random(randNum) * 5.0);
               -- Prepare gudiance system..
               put(Shuttle); put(" call the ball! (Officer 1)"); new_line;
           when Touch_Down =>
               put(Shuttle); put(", permission is granted for final approach. (Officer 1)"); new_line;
    ...
    -- This entire procedure represents a critical resource where access must be controlled.
    procedure LandingControlOfficer2( Request: in RequestType; Shuttle: in ShuttleNameType) is
    begin
       ...
       case Request is
           when Permission_To_Land =>
               delay Duration(Random(randNum) * 5.0);  --  0 <= Delay < 5.0 uniformly distributed.
               -- Make decision.
               put(Shuttle); put(" is granted permission to land. (Officer 2)"); new_line;
           when Call_The_Ball =>
               delay Duration(Random(randNum) * 5.0);
               -- Prepare gudiance system..
               put(Shuttle); put(" call the ball! (Officer 2)"); new_line;
           when Touch_Down =>
               put(Shuttle); put(", permission is granted for final approach. (Officer 2)"); new_line;
               delay Duration(Random(randNum) * 10.0);
               -- Wait for shuttle to land.

       end case;
    end LandingControlOfficer2;
    ...
    Officer1 : Boolean := False;
    Officer2 : Boolean := False;
    ...
    begin
        for I in 1..10 loop
           ...
           for Level in 0..1 loop
              Flag(Atlantis) := Level;
              Turn(Level) := Atlantis;
              while Turn(Level) = Atlantis and then ((Flag(Challenger) >= Level or Flag(Ranger) >= Level) or (Officer1 and Officer2)) loop
                 null;
              end loop;
           end loop;

          if Officer1 = False then
              Officer1 := True;
              --Peterson's Algorithm
              Flag(Atlantis) := -1;
              ...
              New_Line(2);
              Officer1 := False;
          elsif Officer2 = False then
              Officer2 := True;
              --Peterson's Algorithm
              Flag(Atlantis) := -1;
              put("Atlantis entering its critical landing section.");  new_line(2);
              initiateLandingSequence:= seconds(clock);
              LandingControlOfficer2( Permission_To_Land, Atlantis );
              LandingControlOfficer2( Call_The_Ball, Atlantis );
              delay Duration(Random(randNum) * 3.0); -- line up shuttle with mother ship
              LandingControlOfficer2( Touch_Down, Atlantis );
              landingComplete := seconds(clock);
              put("Shuttle Atlantis docked in "); put( landingComplete - initiateLandingSequence );
              put(" seconds." ); new_line;
              put("Atlantis leavings its critical section.  Obtain new crew and start next mission.");
    ...
    begin
        for I in 1..10 loop
           ...
           for Level in 0..1 loop
              Flag(Ranger) := Level;
              Turn(Level) := Ranger;
              while Turn(Level) = Ranger and then ((Flag(Atlantis) >= Level or Flag(Challenger) >= Level) or (Officer1 and Officer2)) loop
                 null;
              end loop;
           end loop;
           ...
           if Officer1 = False then
              ...
              put("Ranger entering its critical landing section.");  new_line(2);
              initiateLandingSequence:= seconds(clock);
              LandingControlOfficer2( Permission_To_Land, Ranger );
              LandingControlOfficer2( Call_The_Ball, Ranger );
              delay Duration(Random(randNum) * 3.0); -- line up shuttle with mother ship
              LandingControlOfficer2( Touch_Down, Ranger );
              landingComplete := seconds(clock);
              put("Shuttle Ranger docked in "); put( landingComplete - initiateLandingSequence );
              put(" seconds." ); new_line;
              put("Ranger leavings its critical section.  Obtain new crew and start next mission.");
    ...
    begin
        for I in 1..10 loop
           ...
           for Level in 0..1 loop
              ...
              while Turn(Level) = Challenger and then ((Flag(Atlantis) >= Level or Flag(Ranger) >= Level) or (Officer1 and Officer2)) loop
                 null;
              end loop;
           end loop;
           ...
           if Officer1 = False then
              ...
              put("Challenger entering its critical landing section.");  new_line(2);
              initiateLandingSequence:= seconds(clock);
              LandingControlOfficer2( Permission_To_Land, Challenger );
              LandingControlOfficer2( Call_The_Ball, Challenger );
              delay Duration(Random(randNum) * 3.0); -- line up shuttle with mother ship
              LandingControlOfficer2( Touch_Down, Challenger );
              landingComplete := seconds(clock);
              put("Shuttle Challenger docked in "); put( landingComplete - initiateLandingSequence );
              put(" seconds." ); new_line;
              put("Challenger leavings its critical section.  Obtain new crew and start next mission.");
